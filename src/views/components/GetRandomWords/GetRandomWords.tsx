...
const GetRandomWords: React.FC<GetRandomWordsProps> = ({ randomWords, text }) => {
    ...
    useEffect(() => { setLocalWords(randomWords) }, [randomWords])

    useEffect(() => {

        const handlePushNewWords = () => {

            localText.forEach(word => {
                localWords.filter(item => item.word === word ? dispatch(pushNewWord(item)) : item)
                setLocalWords(prev => prev.map(item => item.word === word ? { ...item, know: true } : item))
            })
        }
    ...
    return (
        <div className={styles.content}>
            ...
            {localWords && localWords.map(item => (
